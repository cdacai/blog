	...
	components: { Header, BlogPasswordDialog, Tocbot, RandomBlog, Tags, Nav, Footer, Introduction},
		...
		computed: {
			...
			rootStyles() {
				...
				const { colors = {}, spacing = {}, typography = {}, borderRadius = {}, transitions = {} } = this.theme || this.themeConfig
				...
				return {
					...
					'--theme-section-title-margin': (spacing.margin && spacing.margin.sectionTitle) || '32px',
...
<style scoped>
	...
	.sidebar {
		...
		min-width: 220px;
		margin-top: var(--theme-sidebar-margin-top, 50px);
	}
...
.sidebar-content:hover {
	background-color: var(--theme-sidebar-hover, var(--theme-card-bg, #ffffffee));
	...
	.article-card:hover {
	  transform: translateY(-2px);
	  background-color: var(--theme-card-hover, rgba(255, 255, 255, 0.9));
	  box-shadow: var(--theme-shadow-hover, 0 12px 40px rgba(0,0,0,0.13));
	}
