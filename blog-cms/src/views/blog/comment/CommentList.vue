<template>
	<div>
		...
		<el-table :data="commentList" :indent="0" :tree-props="{children: 'replyComments'}" style="width: 100%" row-key="id" border>
			...
			<el-table-column label="所在页面" show-overflow-tooltip>
				<template v-slot="scope">
					<el-link type="success" :href="`${settings.blogUrl}/blog/${scope.row.blog.id}`" target="_blank" v-if="scope.row.page===0">{{ scope.row.blog.title }}</el-link>
					<el-link type="success" :href="`${settings.blogUrl}/about`" target="_blank" v-else-if="scope.row.page===1">关于我</el-link>
					<el-link type="success" :href="`${settings.blogUrl}/friends`" target="_blank" v-else-if="scope.row.page===2">友人帐</el-link>
				</template>
			</el-table-column>
...
<script>
	...
	export default {
		...
		methods: {
			...
			//切换评论公开状态（如果切换隐藏，则该评论的所有子评论都修改为同样的隐藏状态）
