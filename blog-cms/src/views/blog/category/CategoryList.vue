<template>
	<div>
		...
		<el-table :data="categoryList">
			...
			<el-table-column label="默认分类" width="100">
				<template v-slot="scope">
					<el-switch v-model="scope.row.isDefault" @change="handleDefaultChange(scope.row)"></el-switch>
				</template>
			</el-table-column>
			<el-table-column label="操作">
				<template v-slot="scope">
					...
					<el-popconfirm title="确定删除吗？" icon="el-icon-delete" iconColor="red" @onConfirm="deleteCategoryById(scope.row.id)">
						<el-button size="mini" type="danger" icon="el-icon-delete" slot="reference" :disabled="scope.row.isDefault">删除</el-button>
					</el-popconfirm>
				</template>
			</el-table-column>
		</el-table>
		...
		<el-dialog title="添加分类" width="50%" :visible.sync="addDialogVisible" :close-on-click-modal="false" @close="addDialogClosed">
			...
			<el-form :model="addForm" :rules="formRules" ref="addFormRef" label-width="80px">
				...
				<el-form-item label="默认分类">
					<el-switch v-model="addForm.isDefault"></el-switch>
				</el-form-item>
			</el-form>
			<!--底部-->
			<span slot="footer">
		...
		<el-dialog title="编辑分类" width="50%" :visible.sync="editDialogVisible" :close-on-click-modal="false" @close="editDialogClosed">
			...
			<el-form :model="editForm" :rules="formRules" ref="editFormRef" label-width="80px">
				...
				<el-form-item label="默认分类">
					<el-switch v-model="editForm.isDefault"></el-switch>
				</el-form-item>
			</el-form>
			<!--底部-->
			<span slot="footer">
...
<script>
	...
	import {getData, addCategory, editCategory, deleteCategoryById, setDefaultCategory} from '@/api/category'
	...
	export default {
		...
		methods: {
			...
			handleDefaultChange(row) {
				setDefaultCategory(row.id).then(res => {
					this.msgSuccess(res.msg)
					this.getData()
				}).catch(() => {
