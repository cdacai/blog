package com.caixiaohu.util.telegram;
...
import com.caixiaohu.config.properties.TelegramProperties;
...
public class TelegramUtils {
	 ...
	 */
	@Retryable(include = {
			RestClientException.class }, maxAttempts = 5, backoff = @Backoff(delay = 5000L, multiplier = 2))
	@Async
	...
	private void send2TelegramApi(String url, Map<String, Object> data) {
		...
		log.debug("send2TelegramApi => url: {}", url);
		log.debug("send2TelegramApi => response: {}", response);
	}
	...
	private void send2ReverseProxy(String url, Map<String, Object> data) {
		...
		ResponseEntity<String> response = sendPostRequest(telegramProperties.getReverseProxyUrl(), httpEntity,
				String.class);
		log.debug("send2ReverseProxy => reverseProxyUrl: {}", telegramProperties.getReverseProxyUrl());
		log.debug("send2ReverseProxy => destUrl: {}", url);
		log.debug("send2ReverseProxy => response: {}", response);
	}
	...
	private <T> ResponseEntity<T> sendPostRequest(String url, HttpEntity httpEntity, Class<T> responseType) {
		if (telegramProperties.getUseProxy()) {
			log.debug("sendPostRequest => use proxy");
			return restTemplateByProxy.postForEntity(url, httpEntity, responseType);
		}
		log.debug("sendPostRequest => unused proxy");
