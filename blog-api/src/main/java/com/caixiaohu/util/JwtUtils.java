...
public class JwtUtils {
	...
	public void setSecretKey(String secretKey) {
		log.debug("Initializing JWT secretKey: {}", secretKey);
		JwtUtils.secretKey = secretKey;
	}

	@Value("${token.expireTime}")
	public void setExpireTime(long expireTime) {
		log.debug("Initializing JWT expireTime: {}", expireTime);
	...
	public static String generateToken(String subject, Collection<? extends GrantedAuthority> authorities) {
		log.info("Generating token - subject: {}, authorities: {}", subject, authorities);
		String authoritiesStr = authorities.stream()
				.map(GrantedAuthority::getAuthority)
				.reduce((a, b) -> a + "," + b)
				.orElse("");
		log.info("Converted authorities to string: '{}'", authoritiesStr);
		...
		String jwt = Jwts.builder()
				...
				.signWith(SignatureAlgorithm.HS512, secretKey)
				.compact();
		log.info("Verified generated token - subject: {}, authorities: {}",
				Jwts.parser()
	...
	public static String generateToken(String subject, List<String> authorities) {
		log.debug("Generating token - subject: {}, authorities: {}", subject, authorities);
		String authoritiesStr = authorities.toString();
		log.debug("Converted authorities to string: '{}'", authoritiesStr);
		return Jwts.builder()
				.setSubject(subject)
				.claim("authorities", authoritiesStr)
				.setExpiration(new Date(System.currentTimeMillis() + expireTime))
