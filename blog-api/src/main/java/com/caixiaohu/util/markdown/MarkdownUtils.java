package com.caixiaohu.util.markdown;
...
import com.caixiaohu.util.markdown.ext.cover.CoverExtension;
import com.caixiaohu.util.markdown.ext.heimu.HeimuExtension;
...
public class MarkdownUtils {
	...
	private static final Set<Extension> headingAnchorExtensions = Collections
			.singleton(HeadingAnchorExtension.create());
	// 转换table的HTML
	...
	private static class CustomAttributeProvider implements AttributeProvider {
		...
		public void setAttributes(Node node, String tagName, Map<String, String> attributes) {
			...
			// 为代码块添加 Prism.js 所需的类名
			if ("pre".equals(tagName)) {
				attributes.put("class", "line-numbers"); // 添加行号支持
				...
				if (existingClass == null) {
					...
					// 如果已经有 language- 前缀，保持不变，否则添加前缀
					if (!existingClass.startsWith("language-")) {
						attributes.put("class", "language-" + existingClass);
					}
				}
			}
		}
